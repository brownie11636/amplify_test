...
export class PortalCommClient {
  ...
  */
  createSocketIO(namespace = "/", options) {

    let socket = io(namespace, { ...options, path: `/${this.VERSION}` });
  ...
  */
  ...
  createService(deviceId = "undifined device", nsp = "/") {
    console.log('create service for device - ', deviceId);
    let socket = this.sockets[nsp];
    this.profile.serviceId = deviceId;
    
    // 어떤 서비스(webrtc, robot ,...)를 시작할 것인지에 대한 태그.
    // 현재는 webrtc밖에 없긴한데, 추후 이 부분 수정해야 할 듯함.
    let startingServiceType = "webrtc";
    let serviceId;
    
    socket.emit("registering service", "POST", this.profile, (res) => {
 
      if (res.status === 'fail') {
        alert('이미 해당 디바이스에서 방송이 진행중이거나, 해당 디바이스에서 중단된 방송을 재개합니다.');
        return;
      }
